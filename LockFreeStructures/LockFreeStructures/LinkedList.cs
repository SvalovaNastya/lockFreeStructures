    ...
    public class LinkedList<TKey, TValue> where TKey : IComparable
    {
        ...
        public bool TryInsert(TKey key, TValue value)
        {
            ...
            while (true)
            {
                ...
                if (oldState.IsMarked || oldState.Next != searchedNodes.RightNode)
                    continue;
                if (
                    Interlocked.CompareExchange(ref searchedNodes.LeftNode.NextReference.State,
                        new State(newNode, false), oldState) == oldState)
                    return true;
            }
        }
        ...
        public bool TryDelete(TKey key)
        {
            while (true)
            {
                ...
                {
                    if (
                        Interlocked.CompareExchange(ref searchedNodes.RightNode.NextReference.State, newState, oldState) ==
                        oldState)
                        break;
                }
            }
            ...
            while (true)
            {
                ...
                while (nextNode.IsMarkedDeleted() || nextNode.Key.CompareTo(key) < 0)
                {
                    if (nextNode.IsMarkedDeleted())
                    {
                        ...
                        if (Interlocked.CompareExchange(ref currentNode.NextReference.State, newState, oldState) !=
                            oldState)
                        {
